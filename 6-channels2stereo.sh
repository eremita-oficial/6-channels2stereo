...
#
# General usage:
# bash 6-channels2stereo.sh [--mp4 | --mkv | --stereo | --force-vencode | --force-aencode | --config=/path/to/config/] <videofile1> [videofile2 ...]
#
# Normalize the files while keeping the original audio channels (5.1 etc):
# bash 6-channels2stereo.sh [--mp4 | --mkv] <FOLDER>
#
# Normalize files by converting to Stereo:
# bash 6-channels2stereo.sh [--mp4 | --mkv | --stereo] <FOLDER>
#
##########
# CONFIG #
##########
CONFIG_DIRECTORY=~/.6-channels2stereo   #OBS: Não é imprescindível, pode-se usar o script sem esse diretório.

SUPPORTED_EXTENSIONS=('mkv' 'avi' 'mp4' '3gp' 'mov' 'mpg' 'mpeg' 'qt' 'wmv' 'm2ts' 'flv' 'webm' 'm4v')

SUPPORTED_GFORMATS=('MPEG-4' 'Matroska' 'WebM')
UNSUPPORTED_GFORMATS=('BDAV' 'AVI' 'Flash Video' 'DivX' 'RealMedia')

SUPPORTED_VCODECS=('AVC' 'AV1' 'VP8' 'VP9' 'MPEG-4 Visual' 'xvid' 'MPEG Video' 'HEVC' 'RealVideo 4')
UNSUPPORTED_VCODECS=()

SUPPORTED_ACODECS=('AAC' 'AAC LC' 'MPEG Audio' 'Vorbis' 'Ogg' 'Opus' 'AC-3' 'DTS' 'E-AC-3' 'PCM' 'TrueHD' 'Cooker')
UNSUPPORTED_ACODECS=()
...
#DEFAULT_ACODEC=aac
DEFAULT_ACODEC_OPTS="-af dynaudnorm=f=150"
DEFAULT_GFORMAT=mkv
#DEFAULT_GFORMAT=mp4
...
print_help() {
	echo "Usage: 6-channels2stereo.sh [--mp4 | --mkv | --stereo | --force-vencode | --force-aencode | --config=/path/to/config/] <videofile1> [videofile2 ...]"
}
...
process_file() {
	...
	# test if it's an `6-channels2stereo` generated file
	if grep -Fxq "`$REALPATH "$FILENAME"`" "$PROCESSED_FILES"; then
		echo '- file was generated by `6-channels2stereo`, skipping'
		return
	fi
	...
	#if [ ! -z "$STEREO" ] && [ ${INPUT_ACHANNELS:-3} -gt 2 ]; then  # <== Linha original
	if [ ! -z "$STEREO" ] && [ ${INPUT_ACHANNELS:-3} -ne 2 ]; then  # <== Modificado por Benneh Carvalho
	...
	if [ "$OUTPUT_VCODEC" = "copy" ] && [ "$OUTPUT_ACODEC" = "copy" ] && [ "$OUTPUT_GFORMAT" = "ok" ]; then
		echo "- file already has the necessary attributes!"
